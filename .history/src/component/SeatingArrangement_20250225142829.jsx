...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup }) => {
    ...
    // Смещение угла для первого стула
    const initialAngle = 90; // Стул начинается с 90 градусов

    // Генерация стульев для стола
    for (let i = 0; i < maxChairs; i++) {
        const angle = angleStep * i + initialAngle; // Добавляем смещение к углу для первого стула
        ...
        const chairStyle = {
            ...
            backgroundImage: "url('/stul.png')", // Правильный путь для public
        ...
        // Переворачиваем стулья на нижней половине (углы от 180° до 360°)
        const isBottomHalf = angle >= 180 && angle < 360;

        if (isBottomHalf) {
            chairStyle.transform = `rotate(${angle + 180}deg)`; // Переворачиваем стул на нижней половине
        } else {
            chairStyle.transform = `rotate(${angle}deg)`; // Верхняя половина без изменений
        }
