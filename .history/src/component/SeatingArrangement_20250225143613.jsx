...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup }) => {
    ...
    for (let i = 0; i < maxChairs; i++) {
        ...
        if (angle >= 0 && angle < 90) {
            chairStyle.transform = `rotate(${angle}deg)`; // Верхняя половина стола (0-90°)
        } else if (angle >= 90 && angle < 180) {
            chairStyle.transform = `rotate(${angle}deg)`; // Правая часть стола (90-180°)
        } else if (angle >= 180 && angle < 270) {
            chairStyle.transform = `rotate(${angle + 180}deg)`; // Нижняя половина стола (180-270°)
        } else {
            chairStyle.transform = `rotate(${angle + 180}deg)`; // Левая часть стола (270-360°)
        }
