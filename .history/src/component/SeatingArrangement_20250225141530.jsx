...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup }) => {
    ...
    for (let i = 0; i < maxChairs; i++) {
        ...
        // Проверка, чтобы перевернуть стулья на нижней половине стола
        const isBottomHalf = angle >= 180 && angle < 360;
        if (isBottomHalf) {
            chairStyle.transform = `rotate(${angle}deg) translate(${radius}px) rotate(${angle + 180}deg)`; // Переворачиваем стул на нижней половине
        }
