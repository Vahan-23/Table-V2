...
const SeatingArrangement = () => {
    ...
    const handleRemoveTable = (tableId) => {
        setTables((prevTables) => {
            // Находим стол, который удаляем
            const tableToRemove = prevTables.find((t) => t.id === tableId);
            
            // Возвращаем людей за этим столом обратно в общий список
            if (tableToRemove) {
                setPeople((prevPeople) => [
                    ...prevPeople,
                    ...tableToRemove.people.filter((p) => p !== null) // Убираем пустые стулья
                ]);
            }
...
// Компонент Table
const Table = ({ table, setTables, handleRemoveTable, draggingGroup, setDraggingGroup, people ,setPeople }) => {
    ...
    return (
        <div ref={drop} className="tableContainer">
            ...
            <button onClick={() => handleRemoveTable(table.id)}>Удалить стол</button>
