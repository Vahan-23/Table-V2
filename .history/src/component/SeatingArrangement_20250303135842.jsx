...
const SeatingArrangement = () => {
    ...
    const saveHall = () => {
        ...
        const updatedHalls = halls.map(hall =>
            hall.id === currentHall.id ? { ...hall, tables } : hall
        );
    ...
    const createNewHall = () => {
        ...
        const tableCount = Math.max(1, newHallTableCount);
        const chairCount = Math.max(1, newHallChairCount);
    ...
    return (
        <div className="hall-management">
            ...
            <div className="hall-controls">
                ...
                <button className="primary-btn" onClick={() => setShowHallModal(true)}>
                    Ստեղծել նոր դահլիճ
                </button>

                <button className="primary-btn" onClick={saveHall} disabled={!currentHall}>
                    Պահպանել դահլիճը
                </button>

                {currentHall && (
                    <button className="secondary-btn" onClick={() => deleteHall(currentHall.id)}>
                        Ջնջել դահլիճը
                    </button>
                )}
            </div>
            ...
            {showHallModal && (
                <div className="fullscreen-popup" onClick={() => setShowHallModal(false)}>
                    <div className="fullscreen-popup-content" onClick={e => e.stopPropagation()}>
                        <h3>Ստեղծել նոր դահլիճ</h3>
                        <input type="text" value={newHallName} onChange={(e) => setNewHallName(e.target.value)} placeholder="Դահլիճի անունը" />
                        <input type="number" value={newHallTableCount} onChange={(e) => setNewHallTableCount(parseInt(e.target.value) || 1)} placeholder="Սեղանների քանակը" />
                        <input type="number" value={newHallChairCount} onChange={(e) => setNewHallChairCount(parseInt(e.target.value) || 1)} placeholder="Աթոռների քանակը" />
                        <button onClick={createNewHall}>Ստեղծել</button>
                        <button onClick={() => setShowHallModal(false)}>Չեղարկել</button>
                    </div>
                </div>
            )}
        </div>
