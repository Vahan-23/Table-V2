...
const SeatingArrangement = () => {
    ...
    const createNewHall = () => {
        ...
        // Преобразуем значения из инпутов в числа
        const tableCount = Math.max(1, parseInt(newHallTableCount, 10) || 10); // По умолчанию 10
        const chairCount = Math.max(1, parseInt(newHallChairCount, 10) || 12); // По умолчанию 12
    
        // Уникальный ID для зала
        const hallId = Date.now();
    
        // Генерация столов с уникальными ID
        const newTables = Array.from({ length: tableCount }, (_, index) => ({
            id: `${hallId}-${index}`, // Уникальный ID
            people: [],
        ...
        setNewHallName('');
        setNewHallTableCount('10'); // Сбрасываем в строку '10', чтобы избежать ошибок
        setNewHallChairCount('12'); // Аналогично
    };
    ...
    const handleTableCountChange = (e) => {
        const value = e.target.value;
        if (value === '') {
            setNewHallTableCount('');
        } else {
            const parsedValue = parseInt(value, 10);
            if (!isNaN(parsedValue)) {
                setNewHallTableCount(parsedValue.toString()); // Сохраняем как строку
            }
        }
    };
    ...
    const handleChairCountChange = (e) => {
        const value = e.target.value;
        if (value === '') {
            setNewHallChairCount('');
        } else {
            const parsedValue = parseInt(value, 10);
            if (!isNaN(parsedValue)) {
                setNewHallChairCount(parsedValue.toString()); // Сохраняем как строку
            }
        }
    };
