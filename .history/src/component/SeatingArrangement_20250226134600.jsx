...
const renderGroups = () => {
    ...
    return Object.entries(groupedPeople).map(([groupName, group], index) => (
        <Group
            key={index}
            group={group}
            groupName={groupName} // отображаем имя группы, которое задано пользователем
...
return (
    <DndProvider backend={HTML5Backend}>
        ...
        <div className="app-header">
            <h1>Планировщик рассадки гостей</h1>
            <div className="header-controls">
                <button onClick={() => setZoom((z) => Math.min(z + 0.1, 2))}>Увеличить +</button>
                <button onClick={() => setZoom((z) => Math.max(z - 0.1, 0.5))}>Уменьшить -</button>
            </div>
        </div>
        ...
        <div className="container">
            ...
            <div className="right-panel" style={{ transform: `scale(${zoom})`, transformOrigin: "center top" }}>
                {/* Фиксированная область для нового стола */}
                ...
                {/* Контейнер для существующих столов */}
