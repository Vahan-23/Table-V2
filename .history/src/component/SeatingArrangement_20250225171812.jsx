...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup, people }) => {
    ...
    const handleChairClick = (index) => {
        setSelectedChairIndex(index);  // Устанавливаем выбранный стул
        setIsPopupVisible(true); // Открываем попап
    };
    ...
    // Фильтруем людей, исключая тех, кто уже сидит за столом
    ...
    for (let i = 0; i < maxChairs; i++) {
        ...
        chairs.push(
            <div
                ...
                className="chair"
                style={chairStyle}
                onClick={() => handleChairClick(i)} // Обработчик для клика по стулу
            >
        ...
        <div ref={drop} className="tableContainer">
            ...
            {isPopupVisible && (
                <Popup
                    ...
                    onClose={() => setIsPopupVisible(false)} // Закрываем попап
                />
            )}
        </div>
    );
