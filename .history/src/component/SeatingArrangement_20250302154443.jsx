...
const SeatingArrangement = () => {
    ...
    const UnseatedPeopleList = ({ people, tables, onDeletePerson }) => {
                ...
                <div className="unseated-people-grid">
                    {unseatedPeople.map((person, index) => (
                        <div key={index} className="unseated-person-card">
                            ...
                            <button
                                onClick={() => onDeletePerson(person.name)}
                    ...
                    {unseatedPeople.length === 0 && (
                        <div className="no-unseated-people">Բոլոր մարդիկ նստած են</div>
                    )}
                </div>
            </div>
        );
    ...
    // Get people who are seated at tables
    const getSeatedPeople = () => {
        const seated = [];
    ...
    // Calculate total counts for the UI
    const totalPeople = people.length + getSeatedPeople().length;
    const unseatedCount = people.length;
    const seatedCount = getSeatedPeople().length;
                    ...
                    <div className="sidebar">
                        <div className="people-stats">
                            <div className="stat-item">
                                <div className="stat-label">Ընդհանուր</div>
                                <div className="stat-value">{totalPeople}</div>
                            </div>
                            <div className="stat-item">
                                <div className="stat-label">Նստած</div>
                                <div className="stat-value">{seatedCount}</div>
                            </div>
                            <div className="stat-item">
                                <div className="stat-label">Կանգնած</div>
                                <div className="stat-value">{unseatedCount}</div>
                            </div>
                        </div>
                        ...
                        <div className="people-list">
                            ...
                            <div className="people-grid">
                                {[...people, ...getSeatedPeople()].map((person, index) => {
                                    const isSeated = getSeatedPeople().some(p => p.name === person.name);
                                    return (
                                        <div key={index} className={`person-card ${isSeated ? 'seated' : ''}`}>
                                            ...
                                            {isSeated && (
                                                <span className="seated-indicator">նստած</span>
                                            )}
                                        </div>
                                    );
                                })}
                                {[...people, ...getSeatedPeople()].length === 0 && (
                                    <div className="no-people">Մարդիկ չկան</div>
                                )}
                            </div>
                        </div>
                    </div>
    ...
    for (let i = 0; i < table.chairCount; i++) {
        ...
        chairs.push(
            <div
                ...
                title={peopleOnTable[i] ? `Սեղմեք, որպեսզի հեռացնեք ${peopleOnTable[i].name}` : "Սեղմեք, որպեսզի ավելացնեք մարդ"}
            >
                {peopleOnTable[i] && (
                    <div
