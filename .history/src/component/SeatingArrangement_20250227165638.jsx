    ...
    const [, drop] = useDrop({
        ...
        drop: (item) => {
            if (table.people.length + item.group.length <= table.chairCount) {
                ...
                // Also clear dragging state if drop failed
                setDraggingGroup(null);
            }
        },
        collect: (monitor) => ({
        ...
        // Add end callback to ensure dragging state is cleared even if drop is canceled
        end: () => {
            setDraggingGroup(null);
        }
    });
...
// NewTable component with clear end handling
const NewTable = ({ draggingGroup, setTables, setDraggingGroup, setPeople }) => {
    const [{ isOver }, drop] = useDrop({
        ...
        drop: (item) => {
            ...
            // Explicitly clear dragging state after successful drop
            setDraggingGroup(null);
        },
        collect: (monitor) => ({
            isOver: monitor.isOver(),
