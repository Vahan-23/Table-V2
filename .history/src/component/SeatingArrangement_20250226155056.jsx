...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup, people, setPeople }) => {
    ...
    const handleChairClick = (index, event) => {
        // Get the position of the clicked chair
        ...
        const chairCenterY = chairRect.top + chairRect.height / 2;
    ...
    for (let i = 0; i < table.chairCount; i++) {
        ...
        chairs.push(
            <div
                ...
                className="chair"
                style={chairStyle}
                onClick={(e) => handleChairClick(i, e)}
            >
    ...
    // Calculate popup position with centering
    const adjustPosition = () => {
        const windowWidth = window.innerWidth;
        const windowHeight = window.innerHeight;
        const popupWidth = 250; // Estimated popup width
        const popupHeight = Math.min(filteredPeople.length * 30 + 100, 300); // Dynamic height based on list size
        
        // Center the popup around the clicked position
        let adjustedTop = popupPosition.top - (popupHeight / 2);
        let adjustedLeft = popupPosition.left - (popupWidth / 2);
        
        // Ensure popup stays within viewport boundaries
        if (adjustedLeft < 0) {
            adjustedLeft = 10;
        } else if (adjustedLeft + popupWidth > windowWidth) {
            adjustedLeft = windowWidth - popupWidth - 10;
        }
        
        if (adjustedTop < 0) {
            adjustedTop = 10;
        } else if (adjustedTop + popupHeight > windowHeight) {
            adjustedTop = windowHeight - popupHeight - 10;
        }
    ...
    const adjustedPosition = isPopupVisible ? adjustPosition() : { top: 0, left: 0 };
        ...
        drop: (item) => {
            if (item.group.length <= 12) {
                const newTable = {
                    id: Date.now(),
                    people: item.group,
                    chairCount: Math.max(item.group.length, 8), // Минимум 8 стульев или по количеству людей
                };

                setTables((prevTables) => [newTable, ...prevTables]); // Добавляем новый стол в начало списка
                setPeople((prevPeople) =>
