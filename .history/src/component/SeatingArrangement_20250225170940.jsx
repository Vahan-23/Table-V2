...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup, people }) => {
    ...
    // Используем useDrop для обработки перетаскивания группы на стол
    const [, drop] = useDrop({
        accept: 'GROUP',
        drop: (item) => {
    ...
    const filteredPeople = people.filter((person) => {
        return !table.people.some((tablePerson) => tablePerson.name === person.name);
    });
