...
const SeatingArrangement = () => {
    ...
    const handleDeleteTable = (tableId) => {
        setTables(tables.filter(table => table.id !== tableId)); // удаляем стол по id
  
        // Обновляем список людей, чтобы не было дублирования
        setPeople(prevPeople => {
          return prevPeople.filter(person => person.tableId !== tableId);
        });
    };
