...
const SeatingArrangement = () => {
    ...
    const handleDeleteTable = (tableId) => {
        setTables((prevTables) => {
          // Получаем людей, которые были за этим столом
          const table = prevTables.find((table) => table.id === tableId);
          const peopleToReassign = table ? table.people : [];
      
          // Удаляем стол
          const updatedTables = prevTables.filter((table) => table.id !== tableId);
      
          // Обновляем людей, добавляем их в unseatedPeople или возвращаем на другие столы
          setPeople((prevPeople) => [...prevPeople, ...peopleToReassign]);
