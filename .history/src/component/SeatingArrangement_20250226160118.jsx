...
const NewTable = ({ draggingGroup, setTables, setDraggingGroup, setPeople , chairCount}) => {
    const [{ isOver }, drop] = useDrop({
        accept: 'GROUP',
        drop: (item) => {
            if (item.group.length <= chairCount) {
                const newTable = {
                    id: Date.now(),
                    people: item.group,
                    chairCount: Math.max(item.group.length, chairCount), // Минимум 8 стульев или по количеству людей
                };
