...
const handleSelectPerson = (person) => {
    if (selectedChairIndex !== null) {
        // Используем table из props
        const tableId = table.id; // Сохраняем ID текущего стола
        
        setTables(prevTables => {
            return prevTables.map(t => {
                // Обновляем только нужный стол по ID
                if (t.id === tableId) {
                    // Создаем новый массив людей для этого стола
                    const newPeople = [...(t.people || [])];
                    
                    // Расширяем массив при необходимости
                    while (newPeople.length <= selectedChairIndex) {
                        newPeople.push(null);
                    }
                    
                    // Обновляем только один элемент
                    newPeople[selectedChairIndex] = person;
                    
                    // Возвращаем обновленный стол
                    return {
                        ...t,
                        people: newPeople
                    };
                }
                // Остальные столы возвращаем без изменений
                return t;
            });
        });
...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup, people, setPeople, onChairClick }) => {
    ...
    const handleSelectPerson = (person) => {
    if (selectedChairIndex !== null) {
        // Здесь важно сохранить порядок столов таким, какой он есть
        setTables(prevTables => {
            // Создаем точную копию предыдущего массива столов
            return prevTables.map(t => {
                // Обновляем только нужный стол
                if (t.id === table.id) {
                    // Создаем новый массив людей для этого стола
                    const newPeople = [...(t.people || [])];
                    ...
                    // Возвращаем обновленный стол, сохраняя все остальные свойства
                    return {
                        ...t,
                        people: newPeople
                    };
