...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup }) => {
    ...
    for (let i = 0; i < maxChairs; i++) {
        ...
        if (isBottomHalf) {
            chairStyle.transform = `rotate(${angle + 180}deg) translate(${radius}px) rotate(${angle + 180}deg)`; // Переворачиваем стул на нижней половине
        } else {
            chairStyle.transform = `rotate(${angle}deg) translate(${radius}px) rotate(${angle}deg)`; // Верхняя половина без изменений
        }
