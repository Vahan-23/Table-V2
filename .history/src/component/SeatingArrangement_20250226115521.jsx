...
const Table = ({ table, setTables, handleDeleteTable, draggingGroup, setDraggingGroup, people ,setPeople }) => {
    ...
    return (
        <div ref={drop} className="tableContainer">
            ...
            {/* Список людей, которые не сидят за столом */}
            <div className="people-not-at-table">
                <h4>Люди, не сидящие за столом:</h4>
                <ul>
                    {filteredPeople.map((person) => (
                        <li key={person.name}>
                            {person.name}
                            <button onClick={() => handleSelectPerson(person)}>Посадить за стол</button>
                        </li>
                    ))}
                </ul>
            </div>
